#!/usr/bin/env python3
"""
Back-action detection by likelihood-ratio test (LRT) --- Fig. 3 of the paper,
...
Output: ``experiments/em_lrt.pdf`` (+ ``experiments/em_lrt.png`` preview).
...
def main(seeds=350, N=400, npow=120):
    ...
    ax2.errorbar(axyg, powm, yerr=powse, fmt="o-", color="C3", capsize=2, lw=1.3,
                 label="power $\\pm$ 95% CI")
    ...
    fig.savefig(OUT / "em_lrt.png", dpi=150)
