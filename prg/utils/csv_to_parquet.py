...
def detect_encoding(file_path, n_bytes=50000):
    ...
    try:
        ...
        logger.info(
            f"Encodage détecté : {encoding} (confiance={result.get('confidence',0):.2f})"
        )
        return encoding
    except Exception as e:
        logger.warning(
            f"Impossible de détecter l'encodage, utilisation de 'utf-8'. Erreur: {e}"
        )
        return "utf-8"
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        logger.error(
            "Usage : python3 csv_to_parquet.py <fichier.csv> <fichier.parquet>"
        )
    else:
        csv_file = sys.argv[1]
